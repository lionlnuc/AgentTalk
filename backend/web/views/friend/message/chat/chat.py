...
from web.views.friend.message.memory.updata import update_memory
...
class MessageChatView(APIView):
    ...
    async def run_tts_tasks(self, app, inputs, mq, friend):
        ...
        async with websockets.connect(wss_url, additional_headers=headers) as ws:
            await ws.send(json.dumps({
                ...
                "payload": {
                    ...
                    "parameters": {
                        "text_type": "PlainText",
                        "voice": friend.character.voice,  # 音色
                        "format": "mp3",  # 音频格式
    ...
    def work(self,app,inputs,mq, friend):
        try:
            asyncio.run(self.run_tts_tasks(app, inputs, mq, friend))
        finally:
            mq.put_nowait(None)

    def event_stream(self,app,inputs,friend,message):
        mq = Queue()
        thread = threading.Thread(target=self.work, args=(app, inputs, mq, friend))
        thread.start()
